...
PLAN_LIMITS = {
    "free": {"clients": 3, "projects": 3, "invoices": 5},
    "solo": {"clients": -1, "projects": -1, "invoices": -1},
...
async def get_current_user(credentials: HTTPAuthorizationCredentials = Depends(security)):
    ...
    if doc.get("plan") in ("solo", "agency") and doc.get("subscription_expires_at"):
        exp = doc["subscription_expires_at"]
        if isinstance(exp, str):
            exp = datetime.fromisoformat(exp.replace("Z", "+00:00"))
        if exp.tzinfo is None:
            exp = exp.replace(tzinfo=timezone.utc)
        if datetime.now(timezone.utc) > exp:
            await db.users.update_one({"_id": user_id}, {"$set": {"plan": "free", "subscription_expires_at": None}})
            doc["plan"] = "free"
            doc["subscription_expires_at"] = None
...
async def create_client(data: ClientCreate, user: UserDB = Depends(get_current_user)):
    limit = PLAN_LIMITS[user.plan]["clients"]
    if limit != -1:
        count = await db.clients.count_documents({"user_id": user.id, "status": "active"})
        if count >= limit:
            raise HTTPException(status_code=403, detail=f"Free plan limit reached. You can only have {limit} clients. Upgrade to add more.")
    cid = str(uuid.uuid4())
...
async def create_project(data: ProjectCreate, user: UserDB = Depends(get_current_user)):
    limit = PLAN_LIMITS[user.plan]["projects"]
    if limit != -1:
        count = await db.projects.count_documents({"user_id": user.id, "status": "active"})
        if count >= limit:
            raise HTTPException(status_code=403, detail=f"Free plan limit reached. You can only have {limit} projects. Upgrade to add more.")
    pid = str(uuid.uuid4())
...
async def create_invoice(data: InvoiceCreate, user: UserDB = Depends(get_current_user)):
    limit = PLAN_LIMITS[user.plan]["invoices"]
    if limit != -1:
        count = await db.invoices.count_documents({"user_id": user.id})
        if count >= limit:
            raise HTTPException(status_code=403, detail=f"Free plan limit reached. You can only have {limit} invoices. Upgrade to add more.")
...
async def capture_paypal_order(order_id: str, body: dict, user: UserDB = Depends(get_current_user)):
    ...
    if result.get("status") == "COMPLETED":
        ...
        await db.users.update_one({"_id": user.id}, {"$set": {"plan": plan, "subscription_expires_at": expires_at}})
    return result
...
async def verify_razorpay_payment(body: RazorpayVerifyRequest, user: UserDB = Depends(get_current_user)):
    ...
    expires_at = (datetime.now(timezone.utc) + timedelta(days=30)).isoformat()
    await db.users.update_one({"_id": user.id}, {"$set": {"plan": body.plan, "subscription_expires_at": expires_at}})
